...
- **FR-001**: System MUST determine overdue status for each todo by comparing its due date to the current date.
- **FR-002**: System MUST mark a todo as overdue only when it is incomplete and its due date is earlier than the current date.
...
- **Overdue Status**: A derived state indicating whether a todo item is incomplete and has a due date earlier than the current date.

### Assumptions & Dependencies

- The existing todo model already includes completion status and due date data.
- Date comparisons use the same date interpretation rules already used by the application.
...
- **API Contract Discipline**: Feature relies on existing todo data shape and avoids contract-breaking changes between frontend and backend.
